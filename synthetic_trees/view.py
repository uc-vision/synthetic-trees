...
def view_synthetic_data(data: List[Tuple[Cloud, TreeSkeleton]], line_width=1):
    ...
    for i, item in enumerate(data):
        ...
        geometries.append(ViewerItem(f"{tree_name}_cloud", cloud.to_o3d_cloud(), is_visible=visible))
        geometries.append(ViewerItem(f"{tree_name}_labelled_cloud", cloud.to_o3d_cloud_labelled(), is_visible=visible))
        geometries.append(ViewerItem(f"{tree_name}_skeleton", skeleton.to_o3d_lineset(), is_visible=visible))
        geometries.append(ViewerItem(f"{tree_name}_skeleton_mesh", skeleton.to_o3d_tubes(), is_visible=visible))    
...
def parse_args():
    ...
    parser.add_argument("file_path",
                        help="File Path of tree.npz", default=None, type=Path)
