...
import torch
from synthetic_trees.data_types.tree import repair_skeleton, prune_skeleton
...
def parse_args():
    ...
    parser.add_argument("file_path", help="File Path of tree.npz", type=Path)
...
def relative_distance(tube: Tube, p: tm.vec3, query_radius: ti.f32):
    ...
    return ti.select((dist - r) < query_radius, ti.sqrt(dist_sq) / r, torch.inf)
...
def main():
    ...
    tubes = {
        k: torch.from_numpy(x).to(dtype=torch.float32, device=device)
        for k, x in asdict(np_tubes).items()
    }
    ...
    tube_grid = CountedGrid.from_torch(Grid.fixed_size(bounds, (512, 512, 512)), tubes)
    print(f"Grid Made... {time.time() - start_time}")

    point_grid = DynamicGrid.from_torch(
        Grid.fixed_size(bounds, (512, 512, 512)), torch_geom.Point(points)
    )
    print(f"Grid Made... {time.time() - start_time}")
